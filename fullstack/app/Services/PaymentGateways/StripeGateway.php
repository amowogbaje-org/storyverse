    ...
    public function createCheckoutSession(User $user, SubscriptionPlan $plan): CheckoutSession
    {
        $response = Http::asForm()
            ...
            ->post('https://api.stripe.com/v1/checkout/sessions', [
                ...
                'success_url' => config('app.url').'/subscription?status=success',
                'cancel_url' => config('app.url').'/subscription?status=cancelled',
    ...
    public function createTipCheckoutSession(User $tipper, PenName $penName, float $amount, string $currency): CheckoutSession
    {
        $response = Http::asForm()
            ...
            ->post('https://api.stripe.com/v1/checkout/sessions', [
                ...
                'success_url' => config('app.url')."/authors/{$penName->slug}?tip=success",
                'cancel_url' => config('app.url')."/authors/{$penName->slug}?tip=cancelled",
    ...
    public function createStoryPurchaseCheckoutSession(User $user, Story $story, float $amount, string $currency): CheckoutSession
    {
        $response = Http::asForm()
            ...
            ->post('https://api.stripe.com/v1/checkout/sessions', [
                ...
                'success_url' => config('app.url')."/stories/{$story->slug}?purchase=success",
                'cancel_url' => config('app.url')."/stories/{$story->slug}?purchase=cancelled",
